...
# C. Die Feste Welt-Definition (IDs und GESCHÄTZTE Positionen)
...
def get_axis_points(marker_side_length):
    """Definiert die 3D-Punkte für die Achsen, verschoben an die Ecke."""
    ...
    axis_points_3d = np.float32([
        ...
        [-offset, -offset + axis_length, 0],  # Y-Achse (Grün)
...
def process_video():
    ...
    # Parameter für die 2D-Draufsicht (Karte)
        ...
        if ids is not None and len(ids) >= 2: 
            ...
            if len(obj_points) >= 8:
                ...
                if retval: 
                    ...
                    # 1. Glättung der Pose für den Pfad
                    ...
                    for i in range(len(ids)):
                        ...
                        # Zeichne die Achsen: Rot (X), Grün (Y), Blau (Z)
                        ...
                        cv2.line(frame_visual, p_origin, p_y, (0, 255, 0), 3)  # Grüne Y-Achse
                    ...
                    # Zeichne den Pfad in der Draufsicht (GRÜNER PFAD)
                    path_points_2d_map = []
                    ...
                    if len(path_points_2d_map) > 1:
                        for i in range(1, len(path_points_2d_map)):
                            cv2.line(map_image, path_points_2d_map[i-1], path_points_2d_map[i], (0, 255, 0), 2) # Grün
                        
                    # Zeichne die aktuelle Kameraposition (Weißer Punkt)
                    current_pos_map = path_points_2d_map[-1]
                    cv2.circle(map_image, current_pos_map, 4, (255, 255, 255), -1) 

        # 4. Endgültige Fusion und Anzeige
